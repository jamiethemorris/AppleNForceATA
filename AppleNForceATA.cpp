...
bool AppleNForceATA::start( IOService * provider )
{
	...
	IOLog("%s: NVIDIA %s by MeDevil (CMD 0x%x, CTR 0x%x, IRQ %u, BM 0x%x)\n", getName(),
...
bool AppleNForceATA::ata_sata_connect( void )
{
	...
	if (timeout >= 100) {
		ERROR_LOG("%s: warning: phy connection failed. status=0x%08x\n", getName(), status);
		return false;
	}
...
UInt32 AppleNForceATA::scanForDrives( void )
{
	...
	if (_mmapaddr != NULL)
		if (ata_sata_phy_reset())
			ERROR_LOG("%s: sata phy reset done.\n", getName());
    ...
    *_tfAltSDevCReg = 0x0;
	...
	ERROR_LOG("%s: found %d units.\n", getName(), (int)unitsFound);
